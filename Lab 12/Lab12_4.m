...
rhoold(1)=2-rhoold(2);
% rhoold(N+2)=2-rhoold(N+1); % for part i 
rhoold(N+2)=2*rhoold(N+1)-rhoold(N); % for part ii
...
for n=1:nsteps
   ...
   % boundary condition
   % rhonew(N+2)=2-rhonew(N+1); % for part i
   rhonew(N+2)=2*rhonew(N+1)-rhonew(N); % for part ii
   rhonew(1)=2-rhonew(2);
