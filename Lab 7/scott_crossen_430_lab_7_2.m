...
%T = sin(pi*x/L);
nn=1; % this is the k value for sin(k pi x/L). It applies to 7.2 (C)
T = sin(nn*pi*x/L);
vT = 0;
...
% make plots every skip time steps
   if mod(n,skip)==0
      plot(x,T,'r.',x,sin(nn*pi*x/L)*exp(-D*(nn*pi/L)^2*time),'b-')
      xlabel('x');ylabel('y');
      title(['Staggered Leapfrog Wave: time=' num2str(time)])
      axis([min(x) max(x) -2 2]);
      pause(.1)
   end
   errors(n)=mean(abs(T-sin(nn*pi*x/L)*exp(-D*(nn*pi/L)^2*time)));
end
plot(times,errors,'b-');
title('Error vs. Time');
